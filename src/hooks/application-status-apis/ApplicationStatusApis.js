...
// Fetch application status data by employee campus ID
export const getApplicationStatusData = async (employeeCampusId) => {
  try {
    const response = await apiClient.get(`/application-status/getview/employee-campus/${employeeCampusId}`);
    ...
    if (error.code === 'ECONNABORTED') {
      ...
      console.error('   1. Database query optimization for employeeCampusId:', employeeCampusId);
      ...
      console.error('📊 Server Error Response:', {
        ...
        url: `${BASE_URL}/application-status/getview/employee-campus/${employeeCampusId}`,
      });
    } else if (error.request) {
      console.error('📡 No response received from server');
      console.error('   URL:', `${BASE_URL}/application-status/getview/employee-campus/${employeeCampusId}`);
    }
...
export const useApplicationStatusData = (employeeCampusId) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        setError(null);
        const response = await getApplicationStatusData(employeeCampusId);
